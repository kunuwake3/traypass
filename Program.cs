...
// ────────────────────────────────────────
// Контекст приложения (иконка в трее)
// ────────────────────────────────────────
public sealed class TrayAppContext : ApplicationContext
{
    ...
    private static Icon LoadEmbeddedIcon()
    {
        ...
        var resName  = $"{asm.GetName().Name}.tray_favicon.ico";
    ...
    public static PasswordOptions Load(string path)
    {
        try
        {
            return File.Exists(path)
                ? JsonSerializer.Deserialize<PasswordOptions>(File.ReadAllText(path))
                  ?? new PasswordOptions()
                : new PasswordOptions();
        }
        catch
...
public sealed class SettingsForm : Form
{
    ...
    public SettingsForm(PasswordOptions current)
    {
        ...
        _special = AddCheck("Спец-символы",               current.UseSpecial,
                            (s, _) => { _safe.Enabled = ((CheckBox)s!).Checked; });
        _safe    = AddCheck("Только безопасные",          current.SafeSpecialOnly);
        _safe.Enabled = current.UseSpecial;

        grid.Controls.Add(new Label { Text="Длина:", AutoSize=true, Anchor=AnchorStyles.Left }, 0,
            grid.RowCount);
        _len = new NumericUpDown { Minimum=4, Maximum=128, Value=current.Length };
        grid.Controls.Add(_len, 1, grid.RowCount++);

        grid.Controls.Add(new Label { Text="Префикс:", AutoSize=true, Anchor=AnchorStyles.Left }, 0,
            grid.RowCount);
